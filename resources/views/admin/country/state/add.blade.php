        ...
        <div class="box-header with-border">
          <h3 class="box-title"> {{ __('adminstaticword.Add') }} {{ __('adminstaticword.State') }}</h3>
            <div class="panel-heading">
                  <a href=" {{url('admins/state')}} " class="btn btn-success pull-right owtbtn">< {{ __('adminstaticword.Back') }}</a>
            </div>

            <form id="demo-form2" method="post" enctype="multipart/form-data" action="{{url('admins/state')}}" data-parsley-validate class="form-horizontal form-label-left">
                {{csrf_field()}}
