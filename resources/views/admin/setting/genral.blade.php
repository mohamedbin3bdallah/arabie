<form enctype="multipart/form-data" method="POST" action="{{ route('setting.store') }}">
	...
	<div class="row">
		<div class="col-md-6">
			<label for="exampleInputDetails">{{ __('adminstaticword.whatsapp_mobile') }}:</label>
            <input value="{{ $setting->whatsapp_mobile }}" name="whatsapp_mobile" placeholder="{{ __('adminstaticword.whatsapp_mobile') }}" type="number" min="0" class="{{ $errors->has('whatsapp_phone_number') ? ' is-invalid' : '' }} form-control">
		</div>
	</div>
	<br>
