...
@section('title', __('frontstaticword.reschedule'))
        ...
        <div class="row">
            <div class="col-sm-5 teach-item">
                <div class="item-les remaining">
                    <div class="sec-les">
                        <div class="img"><img src="{{ url('/images/user_img/'.$tutor->user->user_img) }}" alt="Arabia" title="Arabia"></div>
                        <p>{{ __('frontstaticword.your_remaining_balance') }} <br/>{{ __('frontstaticword.with') }} {{$tutor->user->fname}} {{$tutor->user->lname[0]}}.</p>
                        <h4 class="title">{{$user_balance->balance}} {{ __('frontstaticword.hours') }}</h4><a class="bottom" href="#buy-hours" data-toggle="modal">{{ __('frontstaticword.buy_hours') }}</a>
                    </div>
                    <form action="#">
                        <div class="fild"><i class="fas fa-chevron-down iconsel"></i>
                            <select class="form-control" name="hour" autocomplete="off" required>
                                <option> </option>
                                <option value="1">1 {{ __('frontstaticword.hour') }}</option>
                                <option value="2">2 {{ __('frontstaticword.hour') }}</option>
                                <option value="3">3 {{ __('frontstaticword.hour') }}</option>
                                <option value="4">4 {{ __('frontstaticword.hour') }}</option>

                            </select>
                            <label class="floating-label">{{ __('frontstaticword.lesson_duration') }}  </label>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <div class="row">
                    <div class="col-sm-6 fild">
                        <ul class="nav nav-tabs" id="myTab" role="tablist">
                            <li class="nav-item"><a class="nav-link active" id="weekly-tab" data-toggle="tab" href="#weekly" role="tab" aria-controls="weekly" aria-selected="true">{{ __('frontstaticword.one_by_one') }}</a></li>
{{--                            <li class="nav-item"><a class="nav-link" id="one-by-one-tab" data-toggle="tab" href="#one-by-one" role="tab" aria-controls="one-by-one" aria-selected="false">{{ __('frontstaticword.weekly') }}</a></li>--}}
                        </ul>
                    </div>
                </div>
                                            ...
                                            <div class="row">
                                                <form action="/course/appointment/{{$tutor->id}}" method="post" enctype="multipart/form-data" class="bookingForm" onsubmit="myButton.disabled = true;">
                                                    ...
                                                    @while($dayOfWeek < \Carbon\Carbon::today()->addDay(7))
                                                                        ...
                                                                        <nav class="listnum">
                                                                            ...
                                                                            @while($currentTime < $schedule->end_time)
                                                                                ...
                                                                                @if($bookedFlag == false)
                                                                                    ...
                                                                                    @else
                                                                                    <li class="active">{{ __('frontstaticword.booked') }}</li>
                                                                                @endif
                                                        ...
                                                        <div><button type="submit" id="submitBooking" name="myButton" hidden="hidden">{{ __('frontstaticword.search') }}</button></div>
                                                </form>
                    ...
                    @foreach($packages as $package)
                        <form action="/student/package/cart/{{$tutor->id}}/{{$package->id}}" method="get">
                            ...
                            <div class="item-buyhours"><img src="/images/icons/{{$package->icon}}" alt="Arabia" title="Arabia">
                                ...
                                <p class="number">{{ ($tutor->PricePerHour - (($tutor->PricePerHour * $package->discountPercentage) / 100))}} $ {{ __('frontstaticword.per_hour') }}</p>
                                <p class="green">{{ __('frontstaticword.you_save') }} {{ ($tutor->PricePerHour * $package->numOfHours) -(($tutor->PricePerHour - (($tutor->PricePerHour * $package->discountPercentage) / 100)) * $package->numOfHours)}} $</p>
