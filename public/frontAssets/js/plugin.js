...
$(document).ready(function ()
{
    ...
    $(".add").click(function ()
    {
        ...
        $(".col-sm-12.add-fild:last").before(
            '<div class="col-11 col-md-12 add-fild"> <div class="row"> <div class="col-sm-6 fild"><i class="fas fa-chevron-down iconsel"></i> <select class="form-control allLang required" name="Languages[' +
            l++ +
            '][language]" autocomplete="off" autofocus="" required="" aria-required="true"> <option> </option>  </select> <label class="floating-label">Language</label> </div><div class="col-sm-6 fild"><i class="fas fa-chevron-down iconsel"></i> <select class="form-control allLevels required"  name="Languages[' +
            x++ +
            '][level]" autocomplete="off" autofocus="" required="" aria-required="true"> <option> </option>  </select> <label class="floating-label">Level   </label> </div></div><div class="bottom icon-erm remove"><span class="fas fa-times"></span></div></div> '
        );
        $.each(lang, function (index, el)
        {
            option =
                '<option value="' + el.id + '">' + el.isoName + "</option>";
            $(".allLang").append(option);
        });
        $.each(levels, function (index, el)
        {
            option = '<option value="' + el.id + '">' + el.name + "</option>";
            $(".allLevels").append(option);
        });
        // i++;
        // x++;
        // console.log(option)
