    ...
    public function boot()
    {
        try {
            ...
            view()->composer('*',function($view){
					...
					if(Auth::check() && Auth::user()->role == 'user')
					{
						...
						switch ($currentRouteActionFunction)
						{
							...
							case "RescheduleLesson":
								$lesson = Appointment::where('id', request()->id)->first();
								if($lesson) $user_balance = UserTutorBalance::where(['user_id'=>Auth::user()->id,'tutor_id'=>$lesson->instructor_id])->first();
								else $user_balance = 0;
								break;
							default:
					...
					$current_lang = (Session::has('changed_language'))? Session::get('changed_language'):'';
