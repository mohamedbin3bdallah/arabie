...
class QuickUpdateController extends Controller
{
    public function courseQuick($id)
    {
    	...
    	if($course->status ==0)
    	{
    		...
    		return back()->with('success',__('adminstaticword.its_now_active'));
    	}
    	else
    	{
    		DB::table('courses')->where('id','=',$id)->update(['status' => "0"]);
    		return back()->with('delete',__('adminstaticword.its_now_deactive'));
    	}
    }
    ...
    public function userQuick($id)
    {
        ...
        if($users->status ==0)
        {
            ...
            if(env('MAIL_USERNAME')!=null) {
                try{
                    /*sending email*/
                    $x = 'Kindly be informed that your profile status has been changed to Active';
        ...
        else
        {
            ...
            if(env('MAIL_USERNAME')!=null) {
                try{
                    /*sending email*/
                    $x = 'Kindly be informed that your profile status has been changed to Blocked';
    ...
    public function courseabc($id)
    {
        ...
        if($course->featured ==0)
        {
            ...
            return back()->with('success',__('adminstaticword.changed_to_yes'));
        }
        else
        {
            DB::table('courses')->where('id','=',$id)->update(['featured' => "0"]);
            return back()->with('delete',__('adminstaticword.changed_to_no'));
        }
    }
