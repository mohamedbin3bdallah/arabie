...
class ApplyCouponController extends Controller
{
	public function applyslotscoupon(Request $request)
    {
    	...
    	if(isset($coupon)){
			...
			if($usercouponcount == $coupon->limitationForSingleUser) return back()->with('error', __('adminstaticword.coupon_student_max_limit_reached'));
			else
			{
    		if($coupon->expirydate >= $date)
    		{
    			if($coupon->maxusage != 0)
    			{
					...
					if(in_array(1, $return)) return back()->with('success', __('adminstaticword.promo_code_has_been_added_successfully'));
					else return back()->with('fail', __('adminstaticword.for_apply_this_coupon_your_cart_total_should_be_or_greater', ['number' => $coupon->minamount]));
    			}
    			else
    			{
    				return back()->with('error', __('adminstaticword.coupon_max_limit_reached'));
    			}

    		}
    		else
    		{
    			return back()->with('error',__('adminstaticword.coupon_expired'));
    		}
			}
    	}else{
    		return back()->with('error',__('adminstaticword.invalid_coupon'));
    	}
    	...
    	if(isset($coupon)){
    		...
    		if($coupon->expirydate >= $date)
    		{
    			if($coupon->maxusage != 0)
    			{
					...
					if($return == 1) return back()->with('success', __('adminstaticword.promo_code_has_been_added_successfully'));
					else return back()->with('fail', __('adminstaticword.for_apply_this_coupon_your_cart_total_should_be_or_greater', ['number' => $coupon->minamount]));
    			}
    			else
    			{
    ...
    public function remove($cpnid)
    {
        ...
        Cart::where('user_id', '=', Auth::user()->id)
            ->update(['distype' => NULL, 'disamount' => NULL]);
        return back()
            ->with('fail', __('adminstaticword.deleted_successfully'));
