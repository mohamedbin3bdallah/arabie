...
class RaveController extends Controller
{
    ...
    public function initialize()
    {
        ...
        if(!$cartRecord){
            Session::flash('error',__('adminstaticword.cart_record_not_found'));
            return redirect()->back()->with('error', __('adminstaticword.cart_record_not_found'));
        }
    ...
    public function callback()
    {
        ...
        if(isset($data->status) && $data->status =='success'){
            ...
            if(!$transactionRecord || $transactionRecord == null){
                Session::flash('error',__('adminstaticword.transaction_success').' '.__('adminstaticword.but').' '.__('adminstaticword.transaction_record_not_found'));
            ...
            if(!$cartRecord){
                Session::flash('error',__('adminstaticword.transaction_success').' '.__('adminstaticword.but').' '.__('adminstaticword.cart_record_not_found'));
                return redirect('/mylessons/'.$txRef[2]);
            }
            ...
            if($data->data->chargecode !='00' || $data->data->chargecode !='0'){
                Session::flash('error',__('adminstaticword.error_in').' '.__('adminstaticword.charge_code'));
                return redirect('/mylessons/'.$txRef[2]);
            }

            // Confirm that the currency on your db transaction is equal to the returned currency
            if($data->data->currency !='USD'){
                Session::flash('error',__('adminstaticword.error_in').' '.__('adminstaticword.received_currency'));
                return redirect('/mylessons/'.$txRef[2]);
            }

            // Confirm that the db transaction amount is equal to the returned amount
            if($data->data->amount != $transactionRecord->total_amount){
                Session::flash('error',__('adminstaticword.error_in').' '.__('adminstaticword.received_amount'));
                return redirect('/mylessons/'.$txRef[2]);
            }
            ...
            $bbbRequest->merge(['meetingname' =>'Arabie Class Room with '. $appointmentRecord->instructor->user->fname.' '. $appointmentRecord->instructor->user->lname. ' and '. $appointmentRecord->user->fname.' '. $appointmentRecord->user->lname]);
            $bbbRequest->merge(['detail' =>'<b>Arabie Class Room with '. $appointmentRecord->instructor->user->fname.' '. $appointmentRecord->instructor->user->lname. ' and '. $appointmentRecord->user->fname.' '. $appointmentRecord->user->lname.'</b>']);
            ...
            $bbbRequest->merge(['welcomemsg' =>'Welcome to Arabie Class Room']);
            ...
            if(env('MAIL_USERNAME')!=null) {
                try{
                    ...
                    $x = 'Kindly be informed that your lesson with student ('.$request->user->fname.' '. $request->user->lname.')at ('.$request->start_time.') on ('.$request->date.') has been confirmed';
...
//                    dd($request->user);
            ...
            try{
                ...
                $notification->data  = "Studnet just booked a session with you";
                $notification->save();
            ...
            Session::flash('success',__('adminstaticword.payment_received_successfully'));

            return redirect('/mylessons/'.$txRef[2]);

        }else{
            Session::flash('error',__('adminstaticword.transaction_failed'));
...
//            if(env('MAIL_USERNAME')!=null) {
                try{
                    ...
                    if($email_templates != null){
                        ...
                        $x = 'Kindly be informed that your lesson with student ('.auth()->user()->fname.' '. auth()->user()->lname.') will start at ('.$appointmentRecord->start_time.') on ('.$appointmentRecord->date.')';

                        $y = 'Kindly be informed that your lesson with tutor ('.$appointmentRecord->instructor->user->fname.' '. $appointmentRecord->instructor->user->lname.') will start at ('.$correct_time.') on ('.$appointmentRecord->date.')';
                    }
//                    dd($request->user);
                    Mail::to($appointmentRecord->instructor->user->email)->send(new UserAppointment($x, $request, $meetingRecord->id));
                    Mail::to($appointmentRecord->user->email)->send(new UserAppointment($y, $request, $meetingRecord->id));
