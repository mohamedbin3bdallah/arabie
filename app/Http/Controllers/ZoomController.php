...
class ZoomController extends Controller
{
    public function dashboard(Request $request){
      if(Auth::user()->role == 'admin' || Auth::user()->role == 'instructor'){
        ...
        if ($err) {
          ...
          return redirect()->route('zoom.setting')->with('delete',__('adminstaticword.zoom_token_or_email_not_found'));
        }
      }else{
        return abort(403, __('adminstaticword.unauthorized_action'));
      }
    ...
    public function updateToken(Request $request){
      ...
      if($query){
        return redirect()->route('zoom.index')->with('success',__('adminstaticword.updated_successfully'));
      }else{
        return back()->with('deleted',__('adminstaticword.wrongdata'));
      }
    }
    ...
    if($response == ''){
      ...
      return redirect()->route('zoom.index')->with('success',__('adminstaticword.deleted_successfully'));
    }else{
      ...
      $request->validate([
        ...
        $token = Auth::user()->jwt_token;
         ...
         $curl = curl_init();
          ...
          return redirect()->route('zoom.show',$response['id'])->with('success',__('adminstaticword.created_successfully'));
    }
