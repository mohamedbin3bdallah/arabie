...
class SettingController extends Controller
{
    public function genreal()
    {
      $env_files = [
		...
		'PAYMENT_GETWAY' => env('PAYMENT_GETWAY'),
		'WHATSAPP_MOBILE' => env('WHATSAPP_MOBILE'),
...
//dd($active);
    ...
    public function extraupdate($request){
      ...
      $setting = Setting::first();
		...
		$setting->whatsapp_mobile = $request->whatsapp_mobile;
        ...
        if(isset($request->w_email_enable))
        {
          if (env('MAIL_USERNAME')!=null) {
            ...
            return back()->with('delete', __('adminstaticword.update_your_mail_api'));
          }
        }
        else
        {
        ...
        $setting->save();
        return redirect()->route('gen.set')->with('success',__('adminstaticword.updated_successfully'));
    }
    ...
    public function updateMailSetting(Request $request)
    {
        ...
        {
          return back()->with('deleted', __('adminstaticword.not_updated'));
        }
    }
	...
	public function paymentGetway(Request $request)
    {
       $setting = Setting::first();
        ...
        if(isset($request->paymentGetway))
        {
          $setting->payment_getway = $request->paymentGetway;
        }
		else
        {
          $setting->payment_getway = "1";
        }

        $env_update = $this->changeEnv([
          'PAYMENT_GETWAY' => $setting->payment_getway,
        ]);

       $setting->save();

       return redirect()->route('gen.set')->with('success',__('adminstaticword.payment_getway_setting_updated'));
    }
