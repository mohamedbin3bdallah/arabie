...
//            ]);
        if(env('MAIL_USERNAME')!=null && $data->status != $request->status) {
			$m = ($request->status)? 'Shown':'Hidden';
            try{
                $request = $data;
                /*sending email*/
                $x = 'Kindly be informed that your profile status has been changed to '.$m;
...
//                    return back()->with('success','Appointment request sent Successfully ! but Mail will not sent because of error in mail configuration !');
    ...
    public function instructor(Request $request)
    {
        ...
        if(!$users->isEmpty()){
            return back()->with('delete',__('adminstaticword.already_equested'));
        }
        else{
        ...
        return back()->with('success',__('adminstaticword.request_successfully'));
