...
{	
	public function getTutorSlots()
	{
		...
		$slots_alert = __('frontstaticword.slots_alert');
		...
		while($week < 4)
		{
			...
			$html .= '<div><div class="between"><p class="day-ma">'.$week_first_day.' '.__('frontstaticword.'.$week_first_month).' '.$week_first_year.' '.__('frontstaticword.to').' '.$week_last_day.' '.__('frontstaticword.'.$week_last_month).' '.$week_last_year.'</p><p class="timezone"><i class="fas fa-globe-americas"></i> '.__('frontstaticword.tutor_slots_week_text_1').' </p><p class="textsacand"><i class="fas fa-info-circle"></i> '.__('frontstaticword.tutor_slots_week_text_2').'</p></div><div class="num-book"><div class="row">';
			...
			while($dayOfWeek < \Carbon\Carbon::today()->addDay(7))
			{
                    ...
                    if($schedule->day == $dayOfWeek->format('l'))
					{
						...
						while($currentTime < $schedule->getOriginal('end_time'))
						{
							...
							else
							{
								...
								$html .= '<li id="ck-book" class="active"><label><span>'.__('frontstaticword.booked').'</span></label></li>';
							}
                ...
                $html .= '</div></div><div><button type="submit" id="submitBooking" class="submitBooking" name="myButton" hidden="hidden">'.__('frontstaticword.search').'</button></div>';
                $dayOfWeek->addDay(1);
        ...
        $html .= '</div><div class="bot-de bot-card modal_sticky_footer"><button type="submit" class="bottom booknow" id="checkBtn">'.__('frontstaticword.Submit').'</button><span class="bottom clo-item">'.__('frontstaticword.cancel').' </span></div></form></div>';
		echo $html;
	}
	...
	public function getTutorContent()
	{
		...
		{
		$slots_alert = __('frontstaticword.slots_alert');
		$html .= '<p class="timezone"><i class="fas fa-globe-americas"></i> '.__('frontstaticword.tutor_content_text').' </p><div class="calandar-wek"><ul class="nav nav-tabs" id="myTab" role="tablist"><li class="nav-item"><a class="nav-link active" id="item-1-tab" data-toggle="tab" href="#item-1" role="tab" aria-controls="item-1" aria-selected="true"></a></li></ul><div class="tab-content" id="myTabContent">';
		...
		{
			$html.= '<div class="tab-pane fade show active" id="item-1" role="tabpanel" aria-labelledby="item-1-tab"><div class="times-day"><h3 class="title">'.__('frontstaticword.'.$day).'</h3><div class="row" style="margin:0px 9px;">';
		...
		$html .= '<div class="text-center"><button type="submit" class="bottom booknow" id="checkBtn">'.__('frontstaticword.Submit').'</button></div>';
		$html .= '<div class="text-center"><a class="bottom" href="/tutor/page/'.$tutor->id.'">'.__('frontstaticword.view_full_schedule').'</a></div>';
		$html .= '</form></div></div>';
		}
		$html .= '</div>';
		echo $html;
