...
//    	dd($meetings);
    ...
    public function store(Request $request){
        ...
        foreach ($allmeeting as $key => $met) {
            if($request->meetingid == $met->meetingid){
                return back()->with('delete',__('adminstaticword.meeting_is_already_active_with_this_name'))->withInput();
            }
        }

        if($request->modpw == $request->attendeepw){
            return back()->with('delete',__('adminstaticword.attandee_password_and_moderator_password_cannot_be_same'))->withInput();
        }
        ...
        return redirect()->route('bbl.all.meeting')->with('success',__('adminstaticword.created_successfully'));
    }
    ...
    public function update(Request $request,$id){
        ...
        return redirect()->route('bbl.all.meeting')->with('success',__('adminstaticword.updated_successfully'));
    }
    ...
    public function delete($meetingid){
        ...
        if(isset($meeting)){
			...
			$meeting->delete();
            return back()->with('deleted',__('adminstaticword.deleted_successfully'));
        }else{
            return back()->with('deleted',__('adminstaticword.record_not_found'));
        }
    }
    ...
    public function setting(Request $request){
        ...
        if($env_update){
            return back()->with('success',__('adminstaticword.updated_successfully'));
        }else{
            return back()->with('deleted',__('adminstaticword.error'));
        }
    }
    ...
    public function apiCreate($id){
        ...
        $m = BBL::find($id);
        if(!$m){
            return back()->with('error', __('adminstaticword.record_not_found'));
        ...
        if($m->instructor_id != 0) {
            ...
            if( $instructorId != $appointment->instructor_id && Auth::user()->id  != $appointment->user_id){
                ...
                return back()->with('error', __('adminstaticword.you_are_not_a_part_of_this_meeting'));
...
//        dd($min15);
        ...
        if ($response->getReturnCode() == 'FAILED') {
            //dd($response);
            return __('adminstaticword.cant_create_room_please_contact_our_administrator');

        } else {

            $joinMeetingParams = new JoinMeetingParameters($m->meetingid, $m->meetingname, $m->modpw);
            $joinMeetingParams->setUsername(Auth::user()->fname ?? 'Arabie Guest' . ' ' . isset(Auth::user()->lname) ?? '');
...
//                }
                return redirect('/')->with('success',__('adminstaticword.meeting_ended_successfully'));
            }else{
                return redirect('/')->with('success',__('adminstaticword.you_logout_from_meeting_successfully'));
            }

        }else{
            return redirect()->with('delete',__('adminstaticword.no_meeting_exist_with_this_id'));
        }
	...
	*/
	public function endMeeting(Request $request){
		...
		{
			return redirect('/')->with('delete',__('adminstaticword.cant_end_this_Meeting'));
        }
    ...
    public function joinview($meetingid){
        ...
        if($m){
            ...
            return back()->with('deleted',__('adminstaticword.meeting_not_found'));
        }
    }
    ...
    public function apiJoin(Request $request){
        ...
        if($m){
            ...
            if($stamp < $currentDate){
                return back()->with('delete',__('adminstaticword.meeting_is_already_ended'))->withInput($request->except('password'));
            }

            if($request->password != $m->attendeepw){
                return back()->with('delete',__('adminstaticword.invalid_password_please_try_again'))->withInput($request->except('password'));
            }
            ...
            return back()->with('delete',__('adminstaticword.meeting_not_found'));
        }
    ...
    public function detailpage(Request $request, $id)
    {
        ...
        if(!$bbl){
            return redirect('/')->with('delete',__('adminstaticword.meeting_not_found'));
        }
