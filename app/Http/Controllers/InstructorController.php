	...
	public function getTutorDetail(Request $request)
    {
		...
		if(!empty($request->input('id')) && $request->input('id') != 0)
		{
			...
			if($tutor)
			{
				...
				{
					$json_data['data']['education'] .= '<div class="row"><div class="col-md-1"></div><div class="col-md-2"><b>'.__('adminstaticword.period').'</b></div><div class="col-md-2"><b>'.__('adminstaticword.university').'</b></div><div class="col-md-3"><b>'.__('adminstaticword.degree').'</b></div><div class="col-md-4"><b>'.__('adminstaticword.specialty').'</b></div></div>';
					foreach($tutor->education as $key => $education)
					{
						$json_data['data']['education'] .= '<div class="row"><div class="col-md-1"><b>'.($key+1).' </b></div><div class="col-md-2">'.$education->from.'-'.$education->to.'</div><div class="col-md-2">'.$education->university.'</div><div class="col-md-3">'.$education->degree.'</div><div class="col-md-4">'.$education->specialty.'</div></div>';
					}
				}
				...
				{
					$json_data['data']['certificate'] .= '<div class="row"><div class="col-md-1"></div><div class="col-md-2"><b>'.__('adminstaticword.period').'</b></div><div class="col-md-2"><b>'.__('adminstaticword.certificate').'</b></div><div class="col-md-2"><b>'.__('adminstaticword.issued_by').'</b></div><div class="col-md-5"><b>'.__('adminstaticword.Description').'</b></div></div>';
					foreach($tutor->certificate as $key => $certificate)
					{
						$issued_by = 'Issued by';
						$json_data['data']['certificate'] .= '<div class="row"><div class="col-md-1"><b>'.($key+1).' </b></div><div class="col-md-2">'.$certificate->from.'-'.$certificate->to.'</div><div class="col-md-2">'.$certificate->certificate.'</div><div class="col-md-2">'.$certificate->$issued_by.'</div><div class="col-md-5">'.$certificate->description.'</div></div>';
					}
				}
				...
				{
					$json_data['data']['work'] .= '<div class="row"><div class="col-md-1"></div><div class="col-md-2"><b>'.__('adminstaticword.period').'</b></div><div class="col-md-4"><b>'.__('adminstaticword.company').'</b></div><div class="col-md-5"><b>'.__('adminstaticword.Title').'</b></div></div>';
					foreach($tutor->work_experience as $key => $work)
					{
						$json_data['data']['work'] .= '<div class="row"><div class="col-md-1"><b>'.($key+1).' </b></div><div class="col-md-2">'.$work->from.'-'.$work->to.'</div><div class="col-md-4">'.$work->company.'</div><div class="col-md-5">'.$work->title.'</div></div>';
					}
				}
	...
	public function changeTutorData(Request $request)
	{
		...
		if(!empty($request->input('id')) && !empty($request->input('type')) && $request->input('id') > 0 && in_array($request->input('to'), [0,1]))
		{
			...
			switch($type)
			{
				...
				case "tutorStatus":
					$tutor = Instructor::find($request->input('id'));
					if($tutor) $update = $tutor->update(['status'=>$request->input('to')]);
					else $error = 1;
					break;
				case "recommendation":
					$tutor = Instructor::find($request->input('id'));
					if($tutor) $update = $tutor->update(['recommendation'=>$request->input('to')]);
					else $error = 1;
					break;
				case "delete":
					$tutor = Instructor::find($request->input('id'));
...
//        event(new StatusNotification($tutor, $notification));
        ...
        if ($tutor == null){
            Session::flash('success',__('adminstaticword.please_complete_your_profile_to_be_a_valid_tutor'));
		...
		$preferredStudentAges = PreferredStudentAge::all();
		$preferredStudentLevels = PreferredStudentLevel::all();
        $levels = DB::table('language_level')->where('active', 1)->get();
        ...
        $languagesSpoken = DB::table('user_language')
            ...
            ->join('language_level', 'language_level.id', '=','user_language.level_id')
            ->select('user_language.language_id', 'user_language.level_id', 'all_languages.isoName As language', 'language_level.name As level')->get();
...
//        $tutorSpecialties = Specialties::all();
        ...
        return view('frontend.tutor-profile', compact('tutor', 'countries', 'allLanguages', 'timeZones', 'specialties', 'days', 'languagesSpoken', 'levels', 'preferredStudentAges', 'preferredStudentLevels', 'tutorPaymentInfo','notificationTypes', 'tutorNotificationsSettings', 'tutorEducations', 'tutorCertificates','tutorExperiences'));
...
//dd($request->all());
        ...
        if ($request->page == "about"){
            /*if ($request->mobile == null){
                ...
                if ($validator->fails()) {
                    Session::flash('message', __('adminstaticword.error'));
			...
			if($request->preferredStudentAge && !empty($request->preferredStudentAge))
			{
				foreach($request->preferredStudentAge as $preferredStudentAge)
				{
					$newTutorPreferedStudentAge = new TutorPreferedStudentAge();
					$newTutorPreferedStudentAge->create([
						'tutor_id' => $tutor->id,
						'prefered_student_age_id' => $preferredStudentAge,
						'created_by' => Auth::user()->id
					]);
				}
			}
			if($request->preferredStudentLevel && !empty($request->preferredStudentLevel))
			{
				if(in_array(7, $request->preferredStudentLevel))
				{
					$newTutorPreferedStudentLevel = new TutorPreferedStudentLevel();
					$newTutorPreferedStudentLevel->create([
						'tutor_id' => $tutor->id,
						'prefered_student_level_id' => 7,
						'created_by' => Auth::user()->id
					]);
				}
				else
				{
					foreach($request->preferredStudentLevel as $preferredStudentLevel)
					{
						$newTutorPreferedStudentLevel = new TutorPreferedStudentLevel();
						$newTutorPreferedStudentLevel->create([
							'tutor_id' => $tutor->id,
							'prefered_student_level_id' => $preferredStudentLevel,
							'created_by' => Auth::user()->id
						]);
					}
				}
        ...
        elseif ($request->page == "video") {
            ...
            if($file = $request->file('uploadVideo'))
            {
                ...
                if ($validator->fails()) {
                    ...
                    Session::flash('error',__('adminstaticword.max_video_size_is_number_mb', ['number'=>100]));
            ...
            $validator = Validator::make($request->all(), $validData);
            if ($validator->fails()) {
                Session::flash('error',__('adminstaticword.invalid_video_url'));
                return redirect()->to('tutor/profile');
            }
        ...
        elseif ($request->page == "password"){
            ...
            {
                Session::flash('error',__('adminstaticword.current_password_error'));
            ...
            if ($request->password == $request->vPassword){
                ...
                Session::flash('error',__('adminstaticword.password_not_matched'));
                return redirect()->to('tutor/profile');
            }
            session(['pageTab' => 6]);
            Session::flash('success',__('adminstaticword.updated_successfully'));
        ...
        elseif ($request->page == "addEducation")
        {
            if ($request->from > $request->to){
                Session::flash('error',__('adminstaticword.error_in_education_dates'));
                return redirect()->back();
            }
            ...
            if(env('MAIL_USERNAME')!=null) {
                try{
                    ...
                    /*sending email*/
                    $x = 'Kindly be informed that your documents are currently in review, this may take up to 5 days..';
//                    dd($request->user);
        ...
        elseif ($request->page == "addExperience"){
            if ($request->from > $request->to){
                Session::flash('error',__('adminstaticword.error_in_experience_dates'));
                return redirect()->back();
            }
...
//                    dd($request->user);
        ...
        elseif ($request->page == "addCertificate"){
            if ($request->from > $request->to){
                Session::flash('error',__('adminstaticword.error_in_certificate_dates'));
                return redirect()->back();
            }
