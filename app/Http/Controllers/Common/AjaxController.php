    ...
    public function registrationSteps(Request $request){
        ...
        if($file = $request->file('uploadVideo'))
        {
            ...
            if ($tutor == 0){
				...
				TutorPreferedStudentAge::where('tutor_id', $tutor_id)->delete();
				TutorPreferedStudentLevel::where('tutor_id', $tutor_id)->delete();
...
//            return "Error: You can't cancel this lesson as the different between today and the lesson date is less than 3 days";
			...
			return json_encode(['Error'=> "Error: ".__('adminstaticword.grace_period_for_canceling_a_lesson_text' , ['number'=>1, 'day'=>__('adminstaticword.day')])]);
			...
			if($paymentTransaction)
            {
				if($paymentTransaction->payment_method == 'flutterwave')
				{
					...
					if($data->status !='success'){
						return json_encode(['Error'=> __('adminstaticword.cannot_refund_this_lesson' , ['message'=>$data->data])]);
					}
				...
				elseif($paymentTransaction && $paymentTransaction->payment_method == 'myfatoora')
				{
					...
					if($data->IsSuccess != 1){
						return json_encode(['Error'=> __('adminstaticword.cannot_refund_this_lesson' , ['message'=>$data->Message])]);
					}
